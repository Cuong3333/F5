  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        new URLSearchParams({ user_input: message }),
        {
          headers: {
  ...
  const handleTaskResponse = async (index, response) => {
    if (response === "Yes" && messages[index].text) {
      try {
        ...
        // Kích hoạt refetch trong Tasks bằng cách thay đổi trạng thái triggerRefetch
...
//         // Gửi tiêu đề nhiệm vụ tới backend để lưu với token trong headers
//         await axios.post("http://localhost:8000/save-task", {
//           title: messages[index].text // Đảm bảo title nằm trong JSON
//         }, {
//           headers: {
//             "Content-Type": "application/json",
